...
const handler: NextApiHandler = async (req, res) => {
  ...
  const fontFilePath = path.join(
    basePath,
    "poppins-v20-latin-ext_latin_devanagari-600.woff",
  );
  ...
  const rawText = String(req.query.slug);
  const text = imageSuffixes.reduce(
    (acc, suffix) =>
  ...
  const isImageRequest = isDiscord || rawText !== text;
  ...
  res.setHeader("Content-Type", "image/svg+xml").status(200).send(resultSvg);
};
